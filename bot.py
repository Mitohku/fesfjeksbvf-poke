...
async def erules(ctx):
	...
	e.add_field(name = "Server Rules", value = "#1. No need for personal attacks. If you have a problem with someone contact one of the Admins/Mods, we will help you.", inline=True)
	e.add_field(name = "ㅤ", value = "#2. Spam of any kind will not be tolerated and may result in a server mute.", inline=True)
	e.add_field(name = "ㅤ", value = "#3. No NSFW content.", inline=True)
	e.add_field(name = "ㅤ", value = "#4. Don't impersonate other players.", inline=True)
	e.add_field(name = "ㅤ", value = f"#5. Don't use commands other than `p!catch` and `p!info` in {general.mention} or {international.mention}.", inline=True)
	e.add_field(name = "ㅤ", value = f"#6. Try to keep swearing to a minimum in {general.mention}, some swearing will be tolerated but try not to.", inline=True)
	e.add_field(name = "ㅤ", value = "#7. Don't advertise your servers in chat or in DMs *(3 day mute in the server, if repeated you will be banned - DM advertising = ban)*", inline=True)
	e.add_field(name = "ㅤ", value = f"#8. {suggestion.mention} is not for having conversations or using the bot commands, it's for suggestions, use the correct channels.", inline=True)
	e.add_field(name = "ㅤ", value = "#9. No racial slurs.", inline=True)
	e.add_field(name = "ㅤ", value = f"#10. Don't tag {berry.mention} or Admins unless there is an urgent issue. Try messaging a Moderator first. Doing so for no reason will result in a 2 hour mute.", inline=True)
	e.add_field(name = "ㅤ", value = "#11. Try not to beg for credits/redeems/pokemons.", inline=True)
	e.add_field(name = "ㅤ", value = "#12. Don't joke about suicide.", inline=True)
	e.add_field(name = "ㅤ", value = f"#13. Keep chat in English or use {international.mention}.", inline=True)
	e.add_field(name = "ㅤ", value = f"#14. Read {faq.mention} and {announcement.mention} before messaging a staff member, chances are the answer to your question is in there.", inline=True)
	e.add_field(name = "ㅤ", value = "#15. Do not repeatedly use a bot command for no reason.", inline=True)
	e.add_field(name = "ㅤ", value = "#16. Follow Discord [Terms of Service](https://discordapp.com/terms) and [Community Guidelines](https://discordapp.com/guidelines)", inline=True)
